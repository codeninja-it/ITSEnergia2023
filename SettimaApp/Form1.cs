...
namespace SettimaApp
{
    public partial class Form1 : Form
    {
        ...
        {
            // sfutto la nostra funzione per avere solo parole "pulite"
            string[] parole = PulisciTesto(testo).Split(" ");
            // tengo la prima intatta
            string intera = parole[0];
            // e stabilisco qual'è la seconda
            string daridurre = parole[1];
            // definisco cosa voglio tenere
            string consonanti = "qwrtpsdfghklzxcvbnm";
            // e mi costruisco una variabile per il risultato
            string ridotta = "";
            // per ogni carattere
            foreach (char carattere in daridurre)
            {
                // se va tenuto
                if (consonanti.Contains(carattere))
                {
                    // lo accodo al risultato
                    ridotta = ridotta + carattere;
                }
            }
            // ed infinite unisco i due segmenti
            return intera + ridotta;
        }
        ...
        {
            txtTesto.Text = Esercizio(txtTesto.Text);
        }
    }
}